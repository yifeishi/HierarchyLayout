...
class Tree(object):
    ...
    class Node(object):
        def __init__(self, box=None, pc_feature=None, left=None, right=None, node_type=None, sym=None, index=None, category=None, reg=None):
            box_np = np.zeros((1,8))
            reg_np = np.zeros((1,8))
            for i in range(8):
                box_np[:,i] = box[i]
    ...
    def __init__(self, scene_dir, feature_dir, boxes, boxes_reg, categories, mapFather, mapChild1, mapChild2, isLeaf, index):
        ...
        while 1:
            for id in range(boxes.shape[0]):
                if isLeaf[id] == 'True' and validNode[id] == 0.0:
                    ...
                    node = Tree.Node(node_type=Tree.NodeType.Leaf, box = boxes[id,:], reg=boxes_reg[id,:], pc_feature = pc_feature, index=id, category=categories[id])
                    mapNode[id] = node
                    validNode[id] = 1
                    ...
                    if mapNode.get(child1, 'False') != 'False' and mapNode.get(child2, 'False') != 'False':
                        pc_feature = self.LoadFeature(feature_dir[id])
                        node = Tree.Node(node_type=Tree.NodeType.Internal, left=mapNode[child1], right=mapNode[child2], box = boxes[id,:], reg=boxes_reg[id,:], pc_feature = pc_feature, index=id, category=categories[id])
                        mapNode[id] = node
                        validNode[id] = 1
            stopFlag = True
            count = 0
...
class GRASSDataset(data.Dataset):
    def __init__(self, dir, transform=None):
        ...
        for pkl_name in pkl_names:
            with open(os.path.join(self.dir,pkl_name), 'r') as f:
    ...
    def __getitem__(self, index):
        ...
        isLeaf = data['isLeaf']
        tree = Tree(scene_dir, feature_dir, boxes, boxes_reg, categories, mapFather, mapChild1, mapChild2, isLeaf, index)
        return tree 
