...
def main():
    ...
    """
    with open(args.path_class_weight, 'r') as f:
        data = pickle.load(StrToBytes(f),encoding='iso-8859-1')
    class_weight = data['class_weight']
    #class_weight = torch.from_numpy(class_weight).float().cuda(config.gpu)
    class_weight[99] = class_weight[0]
            ...
            if epoch_idx_train == num_epochs - 1:
                ...
                for batch_idx_train in range(batch_num_per_epoch):
                    ...
                    for i in range(batch_size):
                        ...
                        # read pickle
                        # weight is related to label
                        #weight.append(class_weight[label_train[index_ch[k]]])
                ...
                for batch_idx_train in range(batch_num_per_epoch):
                    ...
                    for i in range(batch_size):
                        ...
                        #weight.append(class_weight[label_train[index_ch[k]]])
                        weight.append(weight_train[index_ch[k]])
                        #print('label',label_train[index_ch[k]],class_weight[label_train[index_ch[k]]])
                        #weight.append(weight_train[index_ch[k]])
                        size.append(box_sizes[index_ch[k]])
                        data = []
                        count = 0
